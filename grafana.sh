...
while true; do
    ...
    read yn
    case $yn in
        [yY] | [yY][Ee][Ss] ) echo -ne "\e[36mUpdating System - This may take awhile!\e[0m";  (apt-get -y update >/dev/null 2>>install.log && apt-get -y upgrade >/dev/null 2>>install.log;clear) & spinner $! ;echo -e "\r\033[K\e[36mUpdating System ----- Complete\e[0m"; break;; #(Run both in one line)
...
(curl -s https://packagecloud.io/gpg.key | sudo apt-key add - >>/dev/null 2>>install.log) & spinner $!
...
(apt-get install grafana > /dev/null) & spinner $!
...
# Downloading GPG Key - Adding influx to Repo - Clearing APT Cache
echo -ne "\e[36mAdding GPG Key for influx Repo\e[0m"
...
(apt-get update > /dev/null && apt-get install influxdb >>/dev/null 2>>install.log) & spinner $!
